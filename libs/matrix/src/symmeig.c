...
#endif
{
	...
	MatrixReal	*a_ve, *b_ve;
	MatrixReal	b_sqr, bk, ak1, bk1, ak2, bk2, z;
	MatrixReal	c, c2, cs, s, s2, d, mu;
...
#endif
{
	...
	if( SET_MAT_SIZE( A->m, A->n ) < 1000 )
	  mat_get( &tmp, (void *)( MatrixTempBuffer ), A->m, A->n );
	else
	  tmp = matrix_get( A->m, A->n  );

	/*tmp  = m_resize(tmp,A->m,A->n);*/
	tmp  = m_copy(A,tmp);

	if( SET_VEC_SIZE( A->m ) < 1000 ) {
	  vec_get( &b, (void *)(MatrixTempBuffer + 1000), A->m - 1 );
	  vec_get( &diag, (void *)(MatrixTempBuffer + 2000), A->m );
	  vec_get( &beta, (void *)(MatrixTempBuffer + 3000), A->m );
	} else {
	  b   = v_get( A->m - 1 );
	  diag  = v_get( A->m );
	  beta  = v_get( A->m );
...
#endif
*/
	if( beta != (VEC *)(MatrixTempBuffer + 3000 ) ) // память выделялась, надо освободить
	  V_FREE(beta);
	if( diag != (VEC *)(MatrixTempBuffer + 2000) ) // память выделялась, надо освободить
	  V_FREE(diag);
	if( b != (VEC *)(MatrixTempBuffer + 1000) ) // память выделялась, надо освободить
	  V_FREE(b);
	if( tmp != (MAT *)(MatrixTempBuffer) ) // память выделялась, надо освободить
	  M_FREE(tmp);
	return out;
}
