   ...
   if (mem_stat_var[k].var == var) {
	...
	j++;
	 ...
	 if (mem_stat_var[j].var == NULL) return j;
	    ...
	    fprintf(stderr,
              "\n warning_matrix !!! static memory: mem_stat_var is too small\n");
	    fprintf(stderr,
...
*/
int mem_stat_reg_list(void **var, unsigned int type, unsigned int list,
		      char *fname, unsigned int line)
{
   unsigned int n;
   ...
   if ( (int)type < 0 || type >= mem_connect[list].ntypes ||
       mem_connect[list].free_funcs[type] == NULL )
   {
      warning_matrix(WARN_WRONG_TYPE,"mem_stat_reg_list");
      return -1;
   }
   
   if ((int)(n = mem_lookup(var)) >= 0) {
...
int mem_stat_free_list(int mark, int list)
{
   ...
   if (mem_stat_mark_many <= 0) {
      warning_matrix(WARN_NO_MARK,"mem_stat_free");
      return -1;
   }
   ...
   for (i=0; i < mem_hash_idx_end; i++) {
	 ...
	 if (mem_stat_var[j].mark == mark) {
		 ...
		 warning_matrix(WARN_WRONG_TYPE,"mem_stat_free");
